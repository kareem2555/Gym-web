...
export default function ExerciseCar(props) {
  const { exercise, i } = props;

  const [setsCompledted, setSetsCompledted] = useState(0);

  function handleIncrement() {
    setSetsCompledted((setsCompledted + 1) % 6);
    ;
  }
  ...
  bg-slate-950 sm:flex-row 
  sm:items-center sm:flex-wrap gap-x-4"
      >
        <h4
          className="text-3xl hidden sm:inline sm:text-4xl
    md:text-5xl font-semibold text-slate400"
        >
          0{i + 1}
        </h4>
        <h2
          className='capitalize whitespace-nowrap truncate max-w-full text-lg sm:text-xl md:text-2xl flex-1 sm:text-center'
        >
      ...
      >
        {["reps", "rest", "tempo"].map((info) => {
          return (
            ...
            border-solid border-salte-900 w-full"
            >
              <h3 className="capitalize text-slate-400 text-sm">
                {info === `reps` ? exercise.unit : info}
              </h3>
              <p className="font-mdeium">{exercise[info]}</p>
            </div>
          );
        })}
        <button
        ...
        >
          ...
          <p className="font-mdeium">{setsCompledted} / 5</p>
        </button>
      </div>
    </div>
  );
